...
A Technical Analysis employing test of the Unicorn Binance Trailing Stop Loss Engine within Binance's Futures Exchange. This project seeks to computationally define trending channels based on price action patterns typically found visually. Using historic data, we find that exemplary cases defined visually are captured algorithmically, but, unfortunately, a large suite of 'false positives' are generated, too. If we can succesfully subset the set of generated channels,then the program can proceed to test the algorithm's accuracy on real-time data.
